...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QGraphicsScene, \
    QGraphicsView, QGraphicsLinearLayout, QGraphicsWidget, QHBoxLayout, QBoxLayout, QGridLayout, QPlainTextEdit
...
class Window(QWidget):
    def __init__(self):
        ...
        self.widgetSvg.load("/Users/admin/Downloads/деления и цифры.svg")'''
        ...
        self.rb0.load("деления и цифры.svg")
        ...
        self.rb1.load("/Users/admin/Downloads/часовая стрелка.svg")
        proxy_rb1 = self.scene.addWidget(self.rb1)
        proxy_rb1.setRotation(75)
        scene_layout.addItem(proxy_rb1)'''

        renderer = QSvgRenderer("часовая стрелка.svg")
        ...
        renderer2 = QSvgRenderer("минутная стрелка.svg")
        ...
        renderer3 = QSvgRenderer("секундная стрелка.svg")
    ...
    def startCountDown(self,minutes,seconds):
        ...
        self.flickNum = 7
        self.view.setFixedHeight(self.high /1.2)
        self.changeClock(self.high / 1.5)
    ...
    def countdown(self):
        ...
        elif self.countdown_minute>=0>=0:
            self.countdown_second-=1
        if self.countdown_minute>=0:
            self.label1.setText(
                self.addZeros(self.countdown_minute) + ' : <font style="color: rgb(100, 100, 100)">' + self.addZeros(
                    self.countdown_second) + '</font>')

    def flick(self):
        if self.flickNum % 2 ==1 and self.flickNum>0:
            self.scene.setBackgroundBrush(QBrush(Qt.black))
            p = self.palette()
            p.setColor(self.backgroundRole(), Qt.black)
            self.scene.setPalette(p)
            ...
            self.scene.setBackgroundBrush(QBrush(Qt.darkRed))
            p = self.palette()
